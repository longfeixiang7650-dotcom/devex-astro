  ...
  },
{
  ...
  },
  {
    ...
    longDescription:
      `Neovim is a modern, highly extensible fork of Vim designed for today's development workflows. Unlike legacy Vim, Neovim ships with built-in LSP (Language Server Protocol) support, asynchronous job control, and a first-class plugin API—enabling deep IDE-like capabilities without sacrificing performance. Its Lua-based configuration ecosystem (e.g., lazy.nvim, mason.nvim) powers efficient, fast-loading setups that reduce config boot time compared to traditional Vimscript-heavy configurations. Compared to VS Code: Neovim consumes less RAM (avg. 180MB vs. 520MB), starts faster on cold launch, and offers superior keyboard-driven precision—but lacks native GUI debugging or drag-and-drop asset management. Against Sublime Text, Neovim delivers deeper language-agnostic refactoring via Treesitter, yet requires upfront learning to match Sublime's out-of-the-box usability. Neovim is particularly popular among backend engineers (Go, Rust, Python) and DevOps/SREs who leverage its terminal-native editing capabilities.`,
